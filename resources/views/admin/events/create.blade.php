    ...
    <form action="{{ route('admin.events.store') }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded-lg shadow-sm border border-gray-200 mt-2">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-5 mb-4">
            ...
            <div>
                ...
                <input type="number" name="price" min="0" class="w-full border border-gray-300 p-2.5 rounded @error('price') border-red-500 @enderror" required>
                @error('price')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            <div>
                ...
                <input type="number" name="stock" min="1" class="w-full border border-gray-300 p-2.5 rounded @error('stock') border-red-500 @enderror" required>
                @error('stock')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="mb-6">
            ...
            <p class="text-sm text-gray-500 mt-1">Format: JPG, PNG. Maksimal ukuran 2MB</p>
            @error('poster')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
